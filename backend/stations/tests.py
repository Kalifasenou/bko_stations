...
class StationModelTests(TestCase):
    ...
    def setUp(self):
        self.station = Station.objects.create(
            name="Station Test", brand="Shell", latitude=12.6452, longitude=-8.0029
        )
    ...
    def test_get_latest_signalement(self):
        ...
        signalement = Signalement.objects.create(
            station=self.station, fuel_type="Essence", status="Disponible"
        )
    ...
    def test_status_color_red(self):
        ...
        Signalement.objects.create(
            station=self.station, fuel_type="Essence", status="Épuisé"
        )
    ...
    def test_status_color_yellow(self):
        ...
        Signalement.objects.create(
            station=self.station, fuel_type="Gazole", status="Épuisé"
        )
...
class StationAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.staff_user = User.objects.create_user(
            username="apistaff", password="StrongPass123!", is_staff=True
        )
        self.station1 = Station.objects.create(
            name="Station 1", brand="Shell", latitude=12.6452, longitude=-8.0029
        )
        self.station2 = Station.objects.create(
            name="Station 2", brand="Total", latitude=12.6500, longitude=-8.0100
        )
    ...
    def test_available_fuel_filter_keeps_empty_with_recent_signal(self):
        ...
        Signalement.objects.create(
            station=self.station1, fuel_type="Essence", status="Épuisé"
        )
        Signalement.objects.create(
            station=self.station2, fuel_type="Essence", status="Disponible"
        )
    ...
    def test_create_station_with_manager_name(self):
        """Test la création d'une station avec nom du gérant (sans compte)"""
        data = {
    ...
    def test_create_station_with_manager_name_authenticated_non_staff_allowed(self):
        """Un utilisateur authentifié non staff peut aussi créer une station"""
    ...
    def test_create_station_with_manager_name_staff_allowed(self):
        ...
        token_response = self.client.post(
            "/api/auth/token/",
            {"username": "apistaff", "password": "StrongPass123!"},
            format="json",
        )
        access = token_response.data["access"]
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {access}")
...
class SignalementAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        User = get_user_model()
        self.user = User.objects.create_user(
            username="signaluser", password="StrongPass123!"
        )
        self.station = Station.objects.create(
            name="Station Test", brand="Shell", latitude=12.6452, longitude=-8.0029
        )
    ...
    def test_create_signalement_same_status_increments_approval_count(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {access}", HTTP_X_FORWARDED_FOR="11.11.11.11"
        )
        ...
        self.assertEqual(
            Signalement.objects.filter(
                station=self.station, fuel_type="Essence"
            ).count(),
            1,
        )
    ...
    def test_create_signalement_opposite_status_creates_new_event(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {access}", HTTP_X_FORWARDED_FOR="12.12.12.12"
        )
        response = self.client.post(
            "/api/signalements/",
            {"station": self.station.id, "fuel_type": "Essence", "status": "Épuisé"},
        )
        ...
        latest = (
            Signalement.objects.filter(station=self.station, fuel_type="Essence")
            .order_by("-timestamp")
            .first()
        )
...
class ElectricityZoneAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        User = get_user_model()
        self.user = User.objects.create_user(
            username="elecuser", password="StrongPass123!"
        )
    ...
    def test_electricity_by_location(self):
        response = self.client.get(
            "/api/electricity/by-location/?lat=12.6499&lon=-7.9851"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_electricity_by_location_outside_any_zone_returns_none(self):
        response = self.client.get(
            "/api/electricity/by-location/?lat=12.8000&lon=-8.3000"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_electricity_nearby_endpoint(self):
        response = self.client.get(
            "/api/electricity/nearby/?lat=12.6499&lon=-7.9851&radius=10"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_electricity_recommendation_endpoint(self):
        response = self.client.get(
            "/api/electricity/recommendation/?lat=12.6499&lon=-7.9851&radius=10"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_electricity_timeline_endpoint(self):
        response = self.client.get(
            f"/api/electricity/zones/{self.zone.id}/timeline/?hours=24"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class AuthAndMonitoringAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        User = get_user_model()
        self.user = User.objects.create_user(
            username="monitoruser", password="StrongPass123!"
        )
