...
from .permissions import (
    ...
    IsAuthenticatedOrReadOnlyForSignalement,
)
    ...
    def get_queryset(self):
        ...
        if lat and lon:
            try:
                ...
                queryset = (
                    Station.objects.filter(id__in=ids)
                    .annotate(_distance_order=preserved_order)
                    .order_by("_distance_order")
                )
        ...
        if freshness_minutes:
            try:
                ...
                if available_fuel:
                    queryset = queryset.filter(
                        signalements__timestamp__gte=threshold
                    ).distinct()
            except ValueError:
                pass
        ...
        if sort_by == "recent":
            queryset = queryset.annotate(
                last_signalement_at=Max("signalements__timestamp")
            ).order_by("-last_signalement_at", "name")
        elif sort_by == "name":
...
class SignalementViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        existing_vote = Signalement.objects.filter(
            station_id=station_id, ip=ip, timestamp__gte=one_hour_ago
        ).first()
        ...
        data.setdefault(
            ...
            request.data.get("duration_estimate_minutes", 0),
        )
    ...
    def approve(self, request, pk=None):
        ...
        recent_approval = Signalement.objects.filter(
            station=signalement.station, ip=ip, timestamp__gte=one_hour_ago
        ).exists()
...
class ElectriciteSignalementViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        existing_vote = ElectriciteSignalement.objects.filter(
            zone_id=zone_id, ip=ip, timestamp__gte=one_hour_ago
        ).first()
        ...
        four_hours_ago = timezone.now() - timedelta(hours=4)
        existing_signalement = (
            ElectriciteSignalement.objects.filter(
                zone_id=zone_id, timestamp__gte=four_hours_ago
            )
            .order_by("-timestamp")
            .first()
        )
    ...
    def by_zone(self, request):
        ...
        signalements = ElectriciteSignalement.objects.filter(
            zone_id=zone_id, timestamp__gte=timezone.now() - timedelta(hours=4)
...
def electricity_by_location(request):
    ...
    return Response(
        {
            "zone": ZoneElectriqueSerializer(nearest_zone).data
            if nearest_zone
            else None,
            "distance_km": round(min_distance, 2) if min_distance is not None else None,
            "signalement": ElectriciteSignalementSerializer(latest).data
            if latest
            else None,
            "reliability_score": nearest_zone.get_reliability_score()
            if nearest_zone
            else 0,
        }
    )
...
def electricity_nearby(request):
    ...
    for zone in ZoneElectrique.objects.filter(is_active=True):
        ...
        latest = (
            zone.signalements_electricite.filter(timestamp__gte=threshold)
            .order_by("-timestamp")
            .first()
        )
        if (
            ...
            and (not latest or latest.status != status_filter)
        ):
            continue
        ...
        results.append(
            {
                ...
                "latest_signalement": ElectriciteSignalementSerializer(latest).data
                if latest
                else None,
    ...
    if sort_by == "reliability":
        ...
        results.sort(
            key=lambda x: (
                x["latest_signalement"]["timestamp"] if x["latest_signalement"] else ""
            ),
            reverse=True,
        )
    else:
    ...
    return Response(
        {
            "recommendation": {
                ...
                "latest_signalement": ElectriciteSignalementSerializer(
                    best_signalement
                ).data,
                "score": best_score,
            }
...
def electricity_timeline(request, zone_id):
    ...
    timeline = zone.signalements_electricite.filter(timestamp__gte=threshold).order_by(
        "-timestamp"
    )[:200]
...
def electricity_statistics(request):
    ...
    top_zones = active_zones.annotate(
        signalement_count=Count(
            "signalements_electricite",
            filter=Q(signalements_electricite__timestamp__gte=twenty_four_hours_ago),
        )
...
def statistics(request):
    ...
    try:
        ...
        essence_disponible = Signalement.objects.filter(
            timestamp__gte=four_hours_ago, fuel_type="Essence", status="Disponible"
        ).count()

        gazole_disponible = Signalement.objects.filter(
            timestamp__gte=four_hours_ago, fuel_type="Gazole", status="Disponible"
        ).count()

        electricite_disponible = ElectriciteSignalement.objects.filter(
            timestamp__gte=four_hours_ago, status="Disponible"
        ).count()

        electricite_instable = ElectriciteSignalement.objects.filter(
            timestamp__gte=four_hours_ago, status="Instable"
        ).count()
        ...
        top_stations = (
            ...
            .annotate(
                signalement_count=Count(
                    "signalements",
                    filter=Q(signalements__timestamp__gte=twenty_four_hours_ago),
                )
            )
            .filter(signalement_count__gt=0)
            .order_by("-signalement_count")[:5]
        )
        ...
        return Response(
            {
                ...
                "stations_unknown": max(
                    0, total_stations - stations_with_fuel - stations_empty
                ),
...
def brands_list(request):
    ...
    try:
        ...
        return Response({"brands": brands, "count": len(brands)})
    except Exception as e:
    ...
    try:
        ...
        if hours > 168:
            raise ValidationError(
                "Le nombre d'heures ne peut pas dépasser 168 (7 jours)"
            )
        ...
        signalements = (
            Signalement.objects.filter(
                timestamp__gte=time_threshold, fuel_type__in=["Essence", "Gazole"]
            )
            .values("station__latitude", "station__longitude", "status")
...
def login_user(request):
    ...
    if identifier.isdigit():
        profile = (
            UserProfile.objects.select_related("user").filter(phone=identifier).first()
        )
        if profile:
            user = authenticate(
    ...
    if not user.is_active:
        return Response(
            {"error": "Ce compte est désactivé"}, status=status.HTTP_403_FORBIDDEN
        )
...
def monitoring_overview(request):
    ...
    try:
        ...
        return Response(
            {
                "status": "ok"
                if db_status == "connected" and cache_status == "ok"
                else "degraded",
                "database": db_status,
                "cache": cache_status,
                "total_stations": total_stations,
